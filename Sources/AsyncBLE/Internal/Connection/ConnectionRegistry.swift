...
//
// A link lives until something explicitly closes it. Dropping the last app
// reference does not close it — ARC deciding when the radio turns off is not debuggable, and a
// link is device-wide rather than per-caller. So the registry holds a connection strongly until
// it reaches terminal `disconnected`, and lets go then.
//
// Cost accepted, and written down: a link nobody closes leaks. `Central.activeConnections` and
// `Central.disconnectAll()` are how you find and close one.
//
